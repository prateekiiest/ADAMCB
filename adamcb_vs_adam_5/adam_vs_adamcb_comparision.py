...
class ADAMCB:
    ...
    def update_sample_weights(self, S_null):
        with torch.no_grad():
            ...
            # 1. Dynamic L_max (95th percentile of recent gradient norms)
            if len(self.grad_history) > 1000:
                L_max = np.percentile(self.grad_history[-1000:], 95) + 1e-8
            else:
                L_max = np.max(self.gradient_norms.cpu().numpy()) + 1e-8
            
            # 2. Normalize losses to [0.01, 1.0] range using L_max
            # This safely bounds extreme gradient outliers.
            loss_hat = torch.clamp(self.gradient_norms / L_max, min=0.01, max=1.0)
            
            # 3. Zero-mean correction
            # Hard items > 0 (Positive), Easy items < 0 (Negative)
            mu_t = torch.mean(loss_hat)
            centered_loss = loss_hat - mu_t
            
            # 4. Apply empirical Bandit Learning Rate
            # bandit_lr = 0.5 means a max weight jump of ~64% (e^0.5) and max drop of ~39% (e^-0.5)
            bandit_lr = 0.5 
            exponent = bandit_lr * centered_loss 
            exponent = exponent.to(torch.float64)
            
            # Scatter to full dataset array (Unseen items remain exactly 0.0)
            h_hat = torch.zeros(self.n, device=self.device, dtype=torch.float64)
            h_hat.scatter_(0, indices_tensor, exponent)
            
            # 5. Apply multiplicative update
            # Unseen items: w_t * exp(0) -> No change.
            new_w_t = w_temp * torch.exp(h_hat)
            
            # 6. STRICT BOUNDARIES (Replaces Global Normalization)
            # Max=1.0 ensures unseen data is never surpassed.
            # Min=0.1 ensures easy data is not permanently discarded.
            ...
            # 7. Update gradient history
            self.grad_history.extend(self.gradient_norms.cpu().numpy().tolist())
            if len(self.grad_history) > 5000:
                self.grad_history = self.grad_history[-5000:] 
