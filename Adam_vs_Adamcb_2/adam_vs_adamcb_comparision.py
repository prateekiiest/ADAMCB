...
class ADAMCB:
    ...
    def compute_unbiased_gradient_estimate(self):
        ...
        # Single draw probability for strictly unbiased gradient scaling
        p_t_batch = p_t_tensor[indices_tensor] / p_t_tensor.sum()
        scaling_factor = 1.0 / (self.n * p_t_batch)
    ...
    def update_sample_weights(self, S_null):
        with torch.no_grad():
            ...
            # 1. Normalize gradients to [0, 1] 
            batch_max = torch.max(self.gradient_norms)
            batch_min = torch.min(self.gradient_norms)
            norm_grads = (self.gradient_norms - batch_min) / (batch_max - batch_min + 1e-8)

            h_hat = torch.zeros(self.n, device=self.device, dtype=torch.float64)
            selected_p_t = p_t_tensor[indices_tensor] 
            
            # 2. Calculate the Gradient-to-Probability Ratio
            grad_p_ratio = (norm_grads ** 2) / (selected_p_t ** 2)
            
            # 3. Inverted Loss: Hardest item gets 0 loss, easier items get positive loss
            max_ratio = torch.max(grad_p_ratio)
            raw_loss = max_ratio - grad_p_ratio
            
            # Scale by probability for Unbiased Importance Sampling
            raw_h_hat = raw_loss / selected_p_t
            
            # ---------------------------------------------------------
            # 4. DYNAMIC RANGE FORCING
            # ---------------------------------------------------------
            max_h_hat = torch.max(raw_h_hat)
            
            # We want the easiest item to decay by exactly 30% of its current weight 
            # in a single step (multiplier = 0.70). You can tune this!
            target_decay_multiplier = 0.70 
            max_target_exponent = float(-np.log(target_decay_multiplier)) 
            
            if max_h_hat > 1e-8:
                scaled_h_hat = (raw_h_hat / max_h_hat) * max_target_exponent
            else:
                scaled_h_hat = raw_h_hat
                
            h_hat.scatter_(0, indices_tensor, scaled_h_hat)
            
            # 5. Exponent update
            new_w_t = w_temp * torch.exp(-h_hat)
            
            # 6. STRICT BOUNDARIES
            # Max=1.0 ensures unseen data is always at the top of the queue.
            # Min=0.5 strictly enforces your requested floor.
            new_w_t = torch.clamp(new_w_t, min=0.5, max=1.0)
...
if __name__ == '__main__':
    ...
    # --- NEW: Smoothing function to average over every 20 batches ---
    window_size = 20
    ...
    # Generate X-axis (representing the exact step number at the end of each averaged window)
    x_axis = [min(i + window_size, len(adam_train_losses)) for i in range(0, len(adam_train_losses), window_size)]
    
    # Plot Train Loss
    plt.figure(figsize=(10, 6))
    plt.plot(x_axis, adam_train_smooth, label='ADAM', alpha=0.8, linewidth=2)
    plt.plot(x_axis, adamcb_train_smooth, label='ADAMCB', alpha=0.8, linewidth=2)
    plt.xlabel('Step (Batch)')
    plt.ylabel(f'Train Loss (Avg over {window_size} steps)')
    plt.title(f'Train Loss Comparison (Smoothed over {window_size} batches)')
    plt.legend()
    plt.grid(True)
    ...
    # Plot Test Loss
    plt.figure(figsize=(10, 6))
    plt.plot(x_axis, adam_test_smooth, label='ADAM', alpha=0.8, linewidth=2)
    plt.plot(x_axis, adamcb_test_smooth, label='ADAMCB', alpha=0.8, linewidth=2)
    plt.xlabel('Step (Batch)')
    plt.ylabel(f'Test Loss (Avg over {window_size} steps)')
    plt.title(f'Test Loss Comparison (Smoothed over {window_size} batches)')
    plt.legend()
    plt.grid(True)
    ...
    # Plot Test Accuracy
    plt.figure(figsize=(10, 6))
    plt.plot(x_axis, adam_acc_smooth, label='ADAM', alpha=0.8, linewidth=2)
    plt.plot(x_axis, adamcb_acc_smooth, label='ADAMCB', alpha=0.8, linewidth=2)
    plt.xlabel('Step (Batch)')
    plt.ylabel(f'Test Accuracy (%) (Avg over {window_size} steps)')
    plt.title(f'Test Accuracy Comparison (Smoothed over {window_size} batches)')
    plt.legend()
    plt.grid(True)
