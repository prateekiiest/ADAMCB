...
class ADAMCB:
    ...
    def update_sample_weights(self, S_null):
        with torch.no_grad():
            ...
            # 2. Calculate the Importance-Weighted Gradient Signal
            raw_signal = (norm_grads ** 2) / (selected_p_t ** 2)
            
            # 3. Strict Batch Normalization [0, 1]
            sig_max = torch.max(raw_signal)
            sig_min = torch.min(raw_signal)
            scaled_reward = (raw_signal - sig_min) / (sig_max - sig_min + 1e-8)
            
            # 4. STRICT MULTIPLIER BOUNDS
            # We want the hardest item's weight to increase by a maximum factor of 10.
            # ln(10) = 2.302. This is the maximum allowed exponent.
            # We completely bypass the unstable (1 / p_t) division.
            max_multiplier = 10.0
            max_exponent = float(np.log(max_multiplier))
            
            exponent_updates = scaled_reward * max_exponent
            
            h_hat = torch.zeros(self.n, device=self.device, dtype=torch.float64)
            h_hat.scatter_(0, indices_tensor, exponent_updates)
            
            # 5. Multiplicative Update
            # Easiest item: scaled_reward=0 -> exp(0) = 1.0 (Weight stays same)
            # Hardest item: scaled_reward=1 -> exp(2.3) = 10.0 (Weight x10)
            new_w_t = w_temp * torch.exp(h_hat)
            
            # 6. GLOBAL NORMALIZATION 
            # Divide all weights by the dataset's max weight. 
            # The hardest item becomes exactly 1.0, and all others smoothly slide down.
            # Because the max multiplier was bounded, they won't crush to zero.
            new_w_t = new_w_t / torch.max(new_w_t)
            
            # Prevent absolute zero to ensure mathematically safe probabilities
