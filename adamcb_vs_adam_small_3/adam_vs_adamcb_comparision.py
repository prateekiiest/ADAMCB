...
class ADAMCB:
    ...
    def compute_unbiased_gradient_estimate(self):
        ...
        # ---------------------------------------------------------
        # ORIGINAL PAPER ESTIMATOR: g_i / (N * p_i)
        # ---------------------------------------------------------
        ...
        for name, param in self.model.named_parameters():
            ...
            # Record the raw un-scaled norm for the Bandit update
    ...
    def update_sample_weights(self, S_null):
        with torch.no_grad():
            ...
            # 1. Your Insight: The Linear Bandit Signal (g / p)
            # This correctly scales the gradient by how rarely it was sampled.
            bandit_signal = grad_norms / selected_p_t
            
            # 2. Zero-Mean Centering (The Upgrade)
            # Instead of adding (L / p_min) which forces everything down, 
            # we subtract the batch mean. 
            # Hard items > 0 (Positive) | Easy items < 0 (Negative)
            mean_signal = torch.mean(bandit_signal)
            centered_signal = bandit_signal - mean_signal
            
            # 3. Dynamic Normalization
            # We scale the signals strictly between [-1.0, 1.0] so the math never explodes,
            # regardless of whether the gradients are massive or tiny.
            max_abs_signal = torch.max(torch.abs(centered_signal)) + 1e-8
            normalized_signal = centered_signal / max_abs_signal
            
            # 4. Exponent Update
            # Positive signal (hard) = weight increases
            # Negative signal (easy) = weight decreases
            # We use a bandit learning rate of 1.0 so weights can double or halve in one step.
            bandit_lr = 1.0
            exponent = bandit_lr * normalized_signal
            
            h_hat_full = torch.zeros(self.n, device=self.device, dtype=torch.float64)
            h_hat_full.scatter_(0, indices_tensor, exponent)
            
            # Multiplicative update
            new_w_t = w_temp * torch.exp(h_hat_full)
            
            # 5. Strict Bounding
            # Clamp between 0.05 and 1.0 to ensure extreme probabilities don't break the system.
            new_w_t = torch.clamp(new_w_t, min=0.05, max=1.0)
            ...
            floor_count = np.sum(self.w_t <= 0.06)
