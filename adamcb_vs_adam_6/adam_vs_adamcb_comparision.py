...
class ADAMCB:
    ...
    def batch_selection_adamcb(self):
        # 1. Use smaller exploration (was 0.4, now 0.1)
        self.gamma = 0.1
        
        # 2. Cap extreme weights to prevent numerical issues (Perplexity Fix)
    ...
    def update_sample_weights(self, S_null):
        with torch.no_grad():
            ...
            # 1. Zero-Mean Correction directly on Raw Gradients
            raw_signal = self.gradient_norms.to(torch.float64)
            mu_t = torch.mean(raw_signal)
            centered_loss = raw_signal - mu_t
            
            # 2. Split-Scaling (Immune to Outliers)
            # We separate the batch into items that were harder than average, and easier than average.
            scaled_loss = torch.zeros_like(centered_loss)
            
            pos_mask = centered_loss > 0
            neg_mask = centered_loss < 0
            
            # Scale positive items so the absolute hardest item is exactly +1.0
            if pos_mask.any():
                max_pos = torch.max(centered_loss[pos_mask]) + 1e-8
                scaled_loss[pos_mask] = centered_loss[pos_mask] / max_pos
                
            # Scale negative items so the absolute easiest item is exactly -1.0
            if neg_mask.any():
                abs_min_neg = torch.abs(torch.min(centered_loss[neg_mask])) + 1e-8
                scaled_loss[neg_mask] = centered_loss[neg_mask] / abs_min_neg
            
            # 3. Aggressive Empirical Bandit Learning Rate
            # ln(10) ≈ 2.3. This guarantees the easiest item drops to 1/10th of its weight, 
            # and the hardest item multiplies by 10x to recover.
            bandit_lr = 2.302 
            exponent = bandit_lr * scaled_loss
            
            # 4. Scatter to full dataset
            ...
            # 6. Strict Boundaries (Unseen items perfectly protected at 1.0)
            new_w_t = torch.clamp(new_w_t, min=0.1, max=1.0)
            
            # Handle S_null clipping bounds
            if len(S_null) > 0:
    ...
    def step(self):
        # 1. Selects the batch (Using the updated np.clip and gamma logic)
        S_null = self.batch_selection_adamcb()
        
        # 2. Gets the gradients for the batch
        loss = self.compute_unbiased_gradient_estimate()
        
        # 3. Updates the Neural Network weights
        self.update_model_parameters()
        
        # 4. Updates the Bandit probabilities (Using the new Variance-Stabilized EXP3 math)
