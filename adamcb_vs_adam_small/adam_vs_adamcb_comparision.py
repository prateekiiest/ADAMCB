...
class ADAMCB:
    ...
    def batch_selection_adamcb(self):
        w_temp = np.clip(self.w_t.copy(), 1e-10, 1e10)
    ...
    def compute_gradient_estimate(self):
        ...
        # ---------------------------------------------------------
        # THE INVERTED SCALING FIX:
        # Instead of scaling hard items down, we scale them UP.
        # If an item has a high probability (it is hard), we multiply its
        # gradient by its probability to force the optimizer to take a 
        # massive step and actually learn it.
        # ---------------------------------------------------------
        p_t_tensor = torch.tensor(self.p_t, device=self.device, dtype=torch.float32)
        indices_tensor = torch.tensor(self.indices, device=self.device, dtype=torch.long)
        
        # Inverted factor: Directly proportional to probability
        p_t_batch = p_t_tensor[indices_tensor] / p_t_tensor.sum()
        inverted_scaling_factor = p_t_batch * self.n 
        ...
        for name, param in self.model.named_parameters():
            ...
            scaled_grad = per_sample_grad * inverted_scaling_factor.view(*view_shape)
    ...
    def update_sample_weights(self, S_null):
        with torch.no_grad():
            ...
            raw_signal = self.gradient_norms.to(torch.float64)
            mu_t = torch.mean(raw_signal)
            centered_loss = raw_signal - mu_t
            
            scaled_loss = torch.zeros_like(centered_loss)
            
            pos_mask = centered_loss > 0
            neg_mask = centered_loss < 0
            
            if pos_mask.any():
                max_pos = torch.max(centered_loss[pos_mask]) + 1e-8
                scaled_loss[pos_mask] = centered_loss[pos_mask] / max_pos
                
            if neg_mask.any():
                abs_min_neg = torch.abs(torch.min(centered_loss[neg_mask])) + 1e-8
                scaled_loss[neg_mask] = centered_loss[neg_mask] / abs_min_neg
            
            bandit_lr = 2.302 
            exponent = bandit_lr * scaled_loss
            
            h_hat = torch.zeros(self.n, device=self.device, dtype=torch.float64)
            h_hat.scatter_(0, indices_tensor, exponent)
            
            new_w_t = w_temp * torch.exp(h_hat)
            new_w_t = torch.clamp(new_w_t, min=0.1, max=1.0)
            ...
            floor_count = np.sum(self.w_t <= 0.11)
            ceil_count = np.sum(self.w_t >= 0.99)
            return floor_count, ceil_count, raw_signal.min().item(), raw_signal.max().item(), raw_signal.mean().item()
    ...
    def step(self):
        ...
        loss = self.compute_gradient_estimate()
