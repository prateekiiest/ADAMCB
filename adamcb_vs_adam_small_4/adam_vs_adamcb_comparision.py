...
class ADAMCB:
    ...
    def compute_unbiased_gradient_estimate(self):
        ...
        # batch_items[i] is (idx, data, target)
        ...
        # =====================================================
        # NEW: Raw per-sample norms for bandit feedback (||g_i||_2)
        # =====================================================
        ...
        # Store RAW per-sample gradient norms for bandit weight updates
        self.gradient_norms = torch.sqrt(per_sample_norm)  # Shape: [K], true ||∇ℓ(θ;z_i)||_2
    
        # =====================================================
        # Adam unbiased estimator stays the same (g_i/(N*p_i))
        # =====================================================
        p_t_tensor = torch.tensor(self.p_t, device=self.device, dtype=torch.float32)
        ...
        for name, param in self.model.named_parameters():
            ...
            # Original averaging division for Adam update
    ...
    def update_sample_weights(self, S_null):
        with torch.no_grad():
            ...
            # ---------------------------------------------------------
            # ALGORITHM 3: EXACT FORMULA
            # l_t(i) = -(g_i^2 / p_i^2) + (L^2 / p_min^2)
            # h_hat(i) = l_t(i) / p_i
            # w = w * exp(-alpha * h_hat)
            # ---------------------------------------------------------
            p_min = self.gamma / self.n
            
            # We must use the batch maximum as the Lipschitz constant L
            L = torch.max(grad_norms) + 1e-8
            
            # Calculate the true Bandit Loss from Algorithm 3
            ...
            alpha_p = (self.gamma / self.n) * np.sqrt(self.K / (self.n * self.t))  # Stabilized
            ...
            # Divide by probability to get the unbiased estimator h_hat
            h_hat_updates = loss_val / selected_p_t
            
            # --- THE CRITICAL FIX FOR ALGORITHM 3 ---
            # The paper states alpha_p = (gamma/n)^3. As proven in the logs, 
            # this makes alpha_p = 10^-12, freezing the algorithm.
            # To make Algorithm 3 work, we must normalize h_hat so the penalty 
            # is mathematically bounded, and use an empirical learning rate.
            
            # max_h_hat = torch.max(torch.abs(h_hat_updates)) + 1e-8
            # normalized_h_hat = h_hat_updates / max_h_hat
            
            # # Empirical alpha_p (Learning Rate) to allow actual weight movement
            # empirical_alpha = 0.5 
            
            # h_hat_full = torch.zeros(self.n, device=self.device, dtype=torch.float64)
            # h_hat_full.scatter_(0, indices_tensor, normalized_h_hat)
            
            # # The exact multiplicative update from Algorithm 3
            # new_w_t = w_temp * torch.exp(-empirical_alpha * h_hat_full)
            
            # # Safely bound the weights to prevent numerical overflow/underflow
            # new_w_t = torch.clamp(new_w_t, min=1e-5, max=1.0)
...
if __name__ == '__main__':
    ...
    # ---------------------------------------------------------
    # Micro-Experiment Setup (800 Total Samples)
    # ---------------------------------------------------------
    subset_indices = torch.randperm(len(full_train))[:800]
    micro_dataset = Subset(full_train, subset_indices)
    
    # 4:1 Split: 640 Train, 160 Test
    train_subset, test_subset = torch.utils.data.random_split(micro_dataset, [640, 160])
    ...
    print(f"\nStarting Micro-Experiment: {total_steps} total batches (640 Train, 160 Test)...")
    ...
    for step in range(1, total_steps + 1):
        ...
        # --- NEW: Compute per-sample gradients for ADAM strictly for logging ---
        ...
        # vmap calculates independent gradients for each sample in the batch without affecting the main graph
        ...
        # -------------------------------------------------
        # 3. PRINT DIAGNOSTICS & DATAFRAMES
        # -------------------------------------------------
        ...
        print(f"ADAM   -> Unique Items Seen: {len(adam_seen_indices)} / 640")
        print(f"ADAMCB -> Unique Items Seen: {len(adamcb_optimizer.seen_indices)} / 640")
        ...
        print("-" * 80)
        print("[ADAM INTERNAL STATE - BATCH GRADIENTS]")
        df_adam_dict = {}
        for i, sample_idx in enumerate(idx_adam.tolist()):
            col_name = f"ID_{sample_idx}"
            df_adam_dict[col_name] = [float(adam_raw_grads[i])]
            
        df_adam = pd.DataFrame(df_adam_dict, index=['Gradient Norm']).T
        df_adam = df_adam.sort_values(by=['Gradient Norm'], ascending=False).T
        df_adam = df_adam.map(lambda x: f"{float(x):.8f}")
        print(df_adam)
        
        print("-" * 80)
        print("[ADAMCB INTERNAL STATE - BATCH WEIGHT UPDATES]")
        df_cb_dict = {}
        for i, sample_idx in enumerate(sampled_indices):
            col_name = f"ID_{sample_idx}"
            df_cb_dict[col_name] = [
                float(prev_w[sample_idx]),
                float(prev_p[sample_idx]),
                float(cb_raw_grads[i]),
                float(new_w[sample_idx]),
                float(new_w[sample_idx] - prev_w[sample_idx]),
                float(new_p[sample_idx]),
                float(new_p[sample_idx] - prev_p[sample_idx])
            ]
        ...
        df_cb = pd.DataFrame(df_cb_dict, index=row_labels).T
        df_cb = df_cb.sort_values(by=['Prob Before', 'Weight Before'], ascending=[False, False]).T
        df_cb = df_cb.map(lambda x: f"{float(x):.8f}")
        print(df_cb)
